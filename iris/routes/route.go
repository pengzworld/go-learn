...
func Configure(app *bootstrap.Bootstrapper) {
	app.PartyFunc("/user1", func(r iris.Party) {
		mvc.Configure(r, func(m *mvc.Application) {
			m.Register(models.NewUserModel())
			m.Handle(new(controllers.UserController)) //http://localhost:8082/user1
		})
		r.PartyFunc("/path2", func(r iris.Party) {
			mvc.Configure(r, func(m *mvc.Application) {
				m.Handle(new(controllers.UserSubController)) //http://localhost:8082/user1/path2/config
			})
		})
	})
